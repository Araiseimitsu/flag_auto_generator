...
- 「最終測定行」に入力した値を基準に内部設定を自動計算します
- 測定不要行: `入力値 + 4`（入力値が 121 の場合、122〜124 行を空けて 125 行目）
- 工具開始行: `測定不要行 + 3`
- 自動測定データ開始行: `測定不要行 + (工具数 * 3) + 6`
...
- 1〜3 行目の基準式は例として次のとおりです（L 列の例。最終測定行が 121 の場合）  
  `=SUMPRODUCT(--(L11:L119<>""),--(MOD(ROW(L11:L119)-ROW(L11),3)=0))` など
- 2 行目は `L12:L120`、3 行目は `L13:L121` のように、3 行目の終端が最終測定行に合います
